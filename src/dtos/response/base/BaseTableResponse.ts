...
export class BaseTableResponse<TItem extends IIdResponse> {
	...
	static fromJson<TItem extends IIdResponse>(json: any, itemFromJson: (j: any) => TItem): BaseTableResponse<TItem> {
		const rawItems = json.items ?? json.Items ?? []
		const items = Array.isArray(rawItems) ? rawItems.map(itemFromJson) : []
