...
<script setup lang="ts">
	...
	const timeMarkers = computed(() => {
		...
		startTime.setMinutes(Math.ceil(startTime.getMinutes() / tickInterval) * tickInterval, 0, 0)
		...
		while (current <= end) {
			...
			markers.push(new TimeMarker(date, `${position}%`, formatTimeLabel(date), weight))
