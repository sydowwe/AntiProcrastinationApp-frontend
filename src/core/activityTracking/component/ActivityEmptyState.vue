<template>
	<div class="d-flex flex-column align-center justify-center text-center pa-8">
		...
		<template v-if="probeState === 'hasDataOutsideWindow'">
			...
			<p class="text-body-2 text-medium-emphasis mb-3">{{ $t('activityTracking.common.emptyInWindowHint') }}</p>
			<VBtn
				size="small"
				variant="outlined"
		...
		<template v-else-if="probeState === 'emptyFullDay'">
			<p class="text-body-1 text-medium-emphasis mb-1">{{ $t('activityTracking.common.emptyDay') }}</p>
			<RouterLink
...
<script setup lang="ts">
	...
	const { probeState = 'idle', settingsRouteName = null } = defineProps<{
