			...
			<VCard class="h-100 rounded-lg px-3 px-md-4 py-4 d-flex flex-column ga-3">
				<div style="display: grid; grid-template-columns: 1fr auto 1fr; align-items: center; gap: 8px;">
					<div class="d-flex flex-column ga-1">
						<div v-if="group.timePeriod.streak > 0" class="d-flex align-center ga-1">
							<VIcon icon="fire-flame-curved" size="15"
							       :color="group.timePeriod.streak >= group.timePeriod.bestStreak ? 'warning' : undefined"></VIcon>
						...
						<div class="d-flex align-center ga-2 flex-wrap">
							<span v-if="group.timePeriod.totalPeriodsElapsed > 0" class="text-caption font-weight-bold"
							      :class="consistencyColor(group.timePeriod)">
								{{ consistencyPct(group.timePeriod) }}%
							</span>
							<VChip v-if="isAtRisk(group)" color="warning" size="x-small" prependIcon="triangle-exclamation" class="text-black font-weight-bold">
								{{ daysUntilReset(group.timePeriod) }}d left
							</VChip>
						</div>
					</div>

					<VSheet class="px-3 py-1 d-flex align-center ga-2" rounded :style="{ backgroundColor: getBgColor(group.timePeriod.color) }">
...
const visibleGroups = computed(() =>
	groupedItems.value.filter(g => !g.timePeriod.isHidden)
)
