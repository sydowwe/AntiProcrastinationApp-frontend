...
Reusable generic filter for any table view. One component handles the toggle button, the active-filter chips bar, and the slide-in drawer. Parent supplies a filter
DTO, a default-factory, chip formatters, and the input fields via slot.

**Component:** `src/_common/component/FilterPanel.vue`
**Reference implementation:** `src/core/employee/employee/view/EmployeeTableView.vue`
...
### Exported types (`import ... from '@/_common/component/FilterPanel.vue'`)
...
File: `src/core/<module>/<feature>/dto/<Entity>.ts`
...
The table view uses `useEntityQuery()` → `fetchFilteredTable()` (POST `/{entity}/filtered-table`). If your entity's API composable doesn't expose
`fetchFilteredTable` yet, add it:

```ts
// src/core/<module>/widget/api/WidgetApi.ts
import { useFetchFilteredTable } from '@/_common/api/useFetchFilteredTable.ts'
import { WidgetFilterDto, WidgetResponse } from '@/core/<module>/widget/dto/Widget.ts'

export function useWidgetQuery() {
  const { loading, fetchFilteredTable } =
    useFetchFilteredTable<WidgetResponse, WidgetFilterDto>({ entityName: 'widget', responseClass: WidgetResponse })
  // ...other queries
...
import { WidgetFilterDto } from '@/core/<module>/widget/dto/Widget.ts'
...
**Enum multi-select** — wrap with `getEnumSelectOptions(EnumObj, 'i18n.prefix')` from `@/_common/composable/general/EnumComposable.ts`. The returned items use
...
**Tri-state boolean** — use `NullFalseTrueCheckbox` from `src/_common/component/inputs/`.
...
- **Initial load** — `BasicTable` emits `@onLoadItems` on mount, so `loadItems` runs once with the empty default filter. Don't call it manually from `onMounted` as
  well.
...
```vue
<!-- src/core/<module>/widget/view/WidgetTableView.vue -->
<template>
<div class="d-flex flex-column w-100 ga-2">
  <FilterPanel
    v-model="filter"
...
import { FilteredTableRequest } from '@/_common/dto/request/base/FilteredTableRequest.ts'
import { WidgetFilterDto, WidgetResponse } from '@/core/<module>/widget/dto/Widget.ts'
import { useWidgetQuery, useCategoryQuery } from '@/core/<module>/widget/api/WidgetApi.ts'
