            ...
            <v-col cols="12" sm="10" md="10" lg="10" class="mt-lg-5 mt-md-3">
                <v-card class="mx-auto" max-width="400">
                    ...
                    <v-list class="py-0 px-3 px-md-4" density="compact" title="To do list" lines="three" select-strategy="classic" variant="tonal">
                        <ToDoListItem v-for="item in items" :toDoListItem="item" @delete="deleteItem" @edit="toDoListDialog.openEdit" @select="select" @un-select="unSelect"></ToDoListItem>
                    </v-list>
                    <div class="d-flex justify-center pa-4">
                        <VBtn width="70%" color="green" @click="toDoListDialog.openCreate">{{ $t('toDoList.add') }}</VBtn>
                    </div>
                </v-card>
            </v-col>
        </v-row>
