<template>
	<div class="py-4 d-flex flex-column align-center">
		<VBtn
			color="success"
			@click="alarmDialog?.openCreate"
		>
			{{ i18n.t('general.add') }}
		</VBtn>
		<VList
			class="d-flex flex-column pa-0 ga-2 mb-4"
			density="compact"
			title="To do list"
			lines="three"
...
<script setup lang="ts">
	...
	const handleIsActiveChanged = (Alarm: Alarm) => {
		...
		const changedItemsIds = isBatchAction
			? selectedItemsIds.value.map((item: number) => ({ id: item }))
			: [{ id: Alarm.id }]
	...
	function deleteAlarm(id: number) {
		if (selectedItemsIds.value.length > 1) {
			...
			deleteEntity(id).then(() => {
				alarmList.value = alarmList.value.filter((item: Alarm) => item.id !== id)
			})
		}
	}
