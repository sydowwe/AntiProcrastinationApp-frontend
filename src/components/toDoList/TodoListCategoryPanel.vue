...
<div class="h-100 d-flex flex-column">
	<div class="d-flex align-center justify-space-between px-3 pt-4 pb-3">
		...
		<VIconBtn class="d-md-none" icon="xmark" size="40" variant="tonal" @click="emit('closeDialog')"/>
		<VIconBtn class="d-none d-md-block" icon="plus" color="success" size="40" variant="tonal" @click="emit('openCreate')"/>
	</div>

	<div class="px-3 pb-2 d-flex ga-3">
		<VIconBtn :icon="categorySortAsc ? 'sort-alpha-up' : 'sort-alpha-down'" size="40" density="comfortable" variant="tonal" @click="emit('toggleSort')"/>
		<VTextField
			v-model="filterName"
			...
			hideDetails
		/>
		<VIconBtn class="d-md-none" icon="plus" color="success" size="40" variant="tonal" @click="emit('openCreate')"/>
	</div>

	<VDivider class="my-2"/>

	<div class="cat-list pa-2">
		<div
			class="cat-item"
		...
		>
			...
			<VIcon icon="list" size="16" class="cat-item__icon"/>
		...
		>
			...
			<div class="cat-item__prepend">
				<VIcon v-if="cat.icon" :icon="cat.icon" size="16" :color="getTextColor(cat.color)"/>
				<span v-else class="cat-item__dot" :style="{ backgroundColor: getBgColor(cat.color) }"/>
			</div>
			<span class="cat-item__name">{{ cat.name }}</span>
			<span v-if="cat.listCount !== null" class="cat-item__count">{{ cat.listCount }}</span>
			<div v-if="cat.id !== -1" class="cat-item__actions" @click.stop>
				<VIconBtn size="32" variant="tonal" icon="pen" @click.stop="emit('openEdit', cat)">
					<VIcon size="16"/>
				</VIconBtn>
				<VIconBtn size="32" variant="tonal" icon="trash" @click.stop="emit('confirmDelete', cat)">
					<VIcon size="16"/>
				</VIconBtn>
			</div>
		</div>
	</div>
	<VSwitch class="mt-auto ml-auto mr-4 mb-2" v-model="hideEmptyCategories" label="Hide empty" color="primary" hideDetails density="compact"/>
</div>
</template>
