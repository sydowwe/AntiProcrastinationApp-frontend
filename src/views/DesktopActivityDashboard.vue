...
const summaryCardsData = computed<SummaryCardsData[] | null>(() => {
	...
	return summaryCardsRaw.value.map((d) =>
...
const primarySessions = computed<TimelineSessionDto[]>(() =>
	timelineRaw.value?.primarySessions.map((s) =>
		new TimelineSessionDto(s.id, s.productName, s.startedAt, s.endedAt, s.durationSeconds, s.totalSeconds)
	) ?? []
)

const detailSessions = computed<TimelineSessionDto[]>(() =>
	timelineRaw.value?.detailSessions.map((s) =>
		new TimelineSessionDto(s.id, s.productName, s.startedAt, s.endedAt, s.durationSeconds, s.totalSeconds)
	) ?? []
)

const backgroundSessions = computed<TimelineSessionDto[]>(() =>
	timelineRaw.value?.backgroundSessions.map((s) =>
		new TimelineSessionDto(s.id, s.productName, s.startedAt, s.endedAt, s.durationSeconds, s.totalSeconds)
	) ?? []
)
...
async function fetchSummaryCards() {
	...
	try {
		summaryCardsRaw.value = await getDesktopSummaryCards(
			new DesktopSummaryCardsRequest(formatDateForApi(date.value), timeFrom.value, timeTo.value, selectedBaseline.value, 4)
		)
	} finally {
...
async function fetchStackedBars() {
	...
	try {
		stackedBarsRaw.value = await getDesktopStackedBars(
			new DesktopStackedBarsRequest(formatDateForApi(date.value), timeFrom.value, timeTo.value, selectedWindowSize.value)
		)
	} finally {
