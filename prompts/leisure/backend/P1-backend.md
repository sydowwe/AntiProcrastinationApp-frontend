...
The readiness board (`src/core/leisure/component/project/ProjectReadinessBoard.vue`) changes exactly
one field — `readinessStatus` — when a card's status control is clicked. Today the frontend has no
status-only route for this entity, so it falls back to `PUT /activity-project-profile/{id}` with the
full existing profile (all fields unchanged except `readinessStatus`). That works, but round-trips
eight fields to change one enum on every click.
...
- Request body:
  ...
  ```
  `readinessStatus`: string, required, one of `"planning" | "needsShopping" | "readyToStart"` (the
  same enum values `ActivityProjectProfileRequest.readinessStatus` already sends today).
