<template>
<div>
	<p v-if="label" class="text-caption text-medium-emphasis mb-2">{{ label }}</p>
	<div class="d-flex flex-wrap ga-2 align-center">
		<VTooltip v-for="color in COLOR_PALETTE" :key="color.name" :text="color.label" location="top" openDelay="300">
			<template v-slot:activator="{ props }">
				...
				>
					<VIcon v-if="model === color.name" icon="check" size="10" :style="{ color: onSwatchColor(color[currentTheme].bg) }"/>
				</button>
			</template>
		</VTooltip>
		<VTooltip text="None" location="top" openDelay="300">
			<template v-slot:activator="{ props }">
				<button
					v-bind="props"
					type="button"
				...
				>
					<VIcon v-if="!model" icon="check" size="10"/>
				</button>
			</template>
		</VTooltip>
	</div>
