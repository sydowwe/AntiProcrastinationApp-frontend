...
export function useGeneralRules() {
	...
	const onlyLettersWithDiacriticsRule = (v: string) =>
		isOnlyLettersWithDiacritics(v) || `Pole môže obsahovať len písmená`

	const lettersWithDiacriticsAndSpecialCharsRule = (v: string) =>
		isLettersWithDiacriticsAndSpecialChars(v) || `Pole môže obsahovať len písmená a špeciálne znaky`
	...
	return {
		requiredRule,
		onlyLettersWithDiacriticsRule,
		lettersWithDiacriticsAndSpecialCharsRule,
		onlyNumbersRule,
		phoneNumberRule,
