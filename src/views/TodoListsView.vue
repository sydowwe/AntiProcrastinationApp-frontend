...
<div style="position: relative; height: 100%; width: 100%;">
	...
	<VDialog v-model="categoryDrawerOpen" contained fullscreen transition="slide-x-transition" class="d-md-none" :scrim="false" style="margin: -12px">
		<VCard height="100%" rounded="0">
			<TodoListCategoryPanel
				:categories :selectedCategoryId :categorySortAsc
				v-model:hideEmpty="hideEmptyCategories" v-model:filterName="categoryFilterName"
	...
	<VRow class="my-lg-2 my-md-1 bg-background">
		...
		<!-- Right panel: lists -->
		<VCol cols="12" md="8" lg="9" class="bg-background">
			<div class="py-4 pt-md-0 d-flex flex-column flex-md-row ga-3 sticky-toolbar">
				<div class="d-flex align-center ga-3">
					<VBtn prependIcon="bars" variant="tonal" class="d-md-none" @click="categoryDrawerOpen = true">Categories</VBtn>
					<VBtn color="primary" appendIcon="plus" class="flex-grow-1 flex-md-grow-0"
					      @click="dialog?.openCreate(selectedCategoryId !== null && selectedCategoryId > 0 ? selectedCategoryId : null)">
			...
			<div v-if="loading" class="d-flex flex-column ga-3">
				<VCard v-for="i in 3" :key="i" elevation="1">
					<VSkeletonLoader type="list-item-two-line"/>
				</VCard>
			</div>

			<template v-else>
				<TransitionGroup name="list-fade" tag="div" class="d-flex flex-column ga-3 ">
					<VCard
						v-for="list in lists"
						:key="list.id"
						elevation="1"
					...
					>
						<div class="pa-4">
							<div class="d-flex align-center ga-3">
								<div v-if="list.icon" class="list-icon-wrap">
									<VIcon :icon="list.icon" size="20" color="primary"/>
								</div>
								<div class="flex-grow-1" style="min-width: 0">
									<div class="text-subtitle-1 font-weight-medium" style="line-height: 1.3">{{ list.name }}</div>
									<div v-if="list.description" class="text-caption text-medium-emphasis mt-1"
									     style="overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">{{ list.description }}
									</div>
								</div>
								<div class="d-flex ga-2 flex-shrink-0">
									<VIconBtn icon="pen" size="40" color="secondaryOutline" variant="tonal" @click.stop="dialog?.openEdit(list)"/>
									<VIconBtn icon="trash" size="40" color="default" variant="tonal" @click.stop="confirmDelete(list)"/>
								</div>
							</div>

							<div v-if="list.itemCount !== null" class="mt-3">
								<div class="d-flex justify-space-between align-center mb-1">
									<span class="text-caption text-medium-emphasis">{{ list.completedCount ?? 0 }} / {{ list.itemCount }} done</span>
									<span class="text-caption font-weight-medium">
										{{ list.itemCount > 0 ? Math.round(((list.completedCount ?? 0) / list.itemCount) * 100) : 0 }}%
									</span>
								</div>
								<VProgressLinear
									:modelValue="list.itemCount > 0 ? ((list.completedCount ?? 0) / list.itemCount) * 100 : 0"
									color="primary"
				...
				<div v-if="lists.length === 0" class="empty-state">
					<VIcon icon="inbox" size="40" class="mb-3" style="opacity: 0.3"/>
					<template v-if="listFilterName?.trim()">
						<p class="text-subtitle-2 font-weight-medium mb-1">{{ $t('toDoList.namedList.emptyFilter') }}</p>
					</template>
					<template v-else-if="selectedCategoryId !== null">
						<p class="text-subtitle-2 font-weight-medium mb-1">{{ $t('toDoList.namedList.emptyCategory') }}</p>
					</template>
					<template v-else>
...
async function loadCategories() {
	categories.value = await fetchCategories(categorySortAsc.value, hideEmptyCategories.value, categoryFilterName.value?.trim() || null);
}
