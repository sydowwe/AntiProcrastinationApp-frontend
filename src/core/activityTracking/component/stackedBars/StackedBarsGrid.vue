	...
	<div class="stacked-bars-container">
		...
		>
			...
			<template
				...
				:key="`y-left-${line.minute}`"
			>
				<div
					class="y-axis-label y-axis-label-left"
					:style="{ gridRow: line.gridRow, gridColumn: 1 }"
				>
					{{ line.label }}
				</div>
			</template>
			...
			<template
				v-for="line in guideLines"
				:key="`y-right-${line.minute}`"
			>
				<div
					class="y-axis-label y-axis-label-right"
					:style="{ gridRow: line.gridRow, gridColumn: totalColumns + 2 }"
				>
					{{ line.label }}
				</div>
			</template>
			...
			<!-- Window separators -->
			<template
				v-for="(separator, index) in windowSeparators"
				:key="`separator-${index}`"
			>
				<div
			...
			<!-- Activity bars -->
			<template
				v-for="bar in barSpans"
				:key="`${bar.windowIndex}-${bar.activityIndex}`"
			>
				<StackedBarColumn
					:data="bar.data"
			...
			<template v-if="isSubDayMultiDay">
				<template
			...
			<template
				v-for="(window, index) in emptyWindows"
				:key="`empty-${index}`"
			>
				<div
					class="empty-window"
...
<script setup lang="ts">
	...
	import type { ProcessedWindow } from '@/core/activityTracking/component/stackedBars/dto/ProcessedWindow'
	import { BarGridSpan } from '@/core/activityTracking/component/stackedBars/dto/BarGridSpan'
	import { GuideLine } from '@/core/activityTracking/component/stackedBars/dto/GuideLine'
